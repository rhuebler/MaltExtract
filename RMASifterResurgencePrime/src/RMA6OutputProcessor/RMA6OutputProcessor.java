...
public class RMA6OutputProcessor {
	...
	private void prepareOutput(HashMap<Integer,NodeProcessor> results, Filter switcher){
		...
		for(int id : results.keySet()){
			RMA6TaxonProcessor taxProcessor;
				if(switcher == Filter.NON){
					taxProcessor = results.get(id).getDefault();
				}else if(switcher == Filter.ANCIENT){
					taxProcessor = results.get(id).getAncient();
				}else if(switcher == Filter.NONDUPLICATES){
					taxProcessor = results.get(id).getNonDuplicate();
				}else {
					taxProcessor = results.get(id).getAncientNonDuplicate();
				}
	...
	public void process(HashMap<Integer,NodeProcessor> results){
